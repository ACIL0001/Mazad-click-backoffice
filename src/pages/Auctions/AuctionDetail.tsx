...
export default function AuctionDetail() {
  ...
  const getAuctionDetails = async (auctionId: string) => {
    try {
      ...
      enqueueSnackbar(t('auctions.errors.loadingDetails'), { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const getAuctionParticipants = async (auctionId: string) => {
    ...
    try {
          ...
          offers
            .map((offer: any) => ({
              name: offer.user?.firstName && offer.user?.lastName 
                ...
                : offer.user?.email || t('auctions.unknownUser'),
      ...
      enqueueSnackbar(t('auctions.errors.loadingParticipants'), { variant: 'error' });
      setParticipants([]);
    } finally {
  ...
  const CreateChat = async () => {
    if (!participants.length || !participants[0] || !participants[0].user) {
      enqueueSnackbar(t('auctions.errors.noValidParticipant'), { variant: 'error' });
      return;
    }
    ...
    try {
      ...
      enqueueSnackbar(t('auctions.errors.failedToCreateChat'), { variant: 'error' });
    } finally {
  ...
  if (!auction) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" minHeight="60vh">
        <Typography variant="h6">{t('auctions.notFound')}</Typography>
      </Box>
    );
  }
  ...
  return (
    <Page title={`${t('auctions.details')} - ${auction.title}`}>
      <Container>
        {/* Winner Banner */}
          ...
          <Box display="flex" alignItems="center" gap={2}>
            {auction.status === BID_STATUS.OPEN && participants.length > 0 && (
              <Button
                ...
                size="large"
              >
                {chatLoading ? <CircularProgress size={22} color="inherit" /> : t('auctions.contactWinner')}
              </Button>
            )}
            <Paper
              elevation={0}
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                >
                  {t('auctions.statusLabel')}
                </Typography>
              </Box>
              <Chip
                label={auction.status === BID_STATUS.OPEN ? t('auctions.status.open') : t(`auctions.status.${auction.status.toLowerCase()}`)}
            ...
            <Card sx={{ p: 2, mb: 3, minHeight: '200px', maxHeight: '250px', display: 'flex', flexDirection: 'column' }}>
              ...
              <Stack spacing={1} sx={{ flex: 1 }}>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    <Typography variant="body2" color="text.secondary">
                      {t('auctions.initialPrice')}
                    </Typography>
                    <Typography variant="h6">{auction.startingPrice.toFixed(2)} {t('currency')}</Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="body2" color="text.secondary">
                      {t('auctions.currentPrice')}
                    </Typography>
                    <Typography variant="h6" color="primary.main">
                      {auction.currentPrice.toFixed(2)} {t('currency')}
                    </Typography>
                  </Grid>
                </Grid>
              </Stack>
                    ...
                    >
                      <Stack direction="row" alignItems="center" spacing={2}>
                        ...
                        <Box flex={1}>
                          ...
                          <Typography variant="body2" color="text.secondary">
                            {t('auctions.bid')}: {participant.bidAmount?.toFixed(2)} {t('currency')}
                          </Typography>
                        </Box>
              ...
              <Stack spacing={2} sx={{ flex: 1, justifyContent: 'space-around' }}>
                <Box>
                  ...
                  <Label variant="ghost" color="default">
                    {auction.bidType === BID_TYPE.PRODUCT ? t('auctions.typeProduct') : t('auctions.typeService')}
                  </Label>
                </Box>
                <Box>
                  <Typography variant="body2" color="text.secondary">
                  ...
                  >
                    {auction.auctionType === AUCTION_TYPE.EXPRESS
                      ? `${t('auctions.modeExpress')} (MEZROUB)`
                      : auction.auctionType === AUCTION_TYPE.AUTO_SUB_BID
                      ? t('auctions.modeAutomatic')
                      : t('auctions.modeClassic')}
                  </Label>
                </Box>
              </Stack>
            </Card>
